  ...
  import: function(config, database, fixture, additive, quiet){
    ...
    return fs.listTree(filepath)
      .then(function(collections){
        ...
        return import_collections(url(connection), connection.database, filepath, collections, additive, quiet);
      })
      .then(function(c){
        if(!quiet){
...
function import_collections(host, database, filepath, collections, additive, quiet){
  return new Promise(function(resolve, reject){
    if(collections.length === 0) resolve(true);
    var collection = collections.shift();
    var collection_info = collection.match(/[\/|\\]([^\/|\\]*).json$/);
    var exec_line ="mongoimport --db="+database+" --host="+host+" --collection="+collection_info[1]+" --file="+collection
    if(!additive){
      exec_line = exec_line+" --drop"
    }
    exec(exec_line, function(err, stdout, stderr ){
      if(err) reject(stderr)
      if(!quiet){
        console.log("\t", collection, "imported");
      }
      import_collections(host, database, filepath, collections, additive, quiet)
        .then(resolve)
    });
  })
