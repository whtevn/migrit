  ...
  import: function(config, database, fixture, additive, quiet){
    ...
    return fs.listTree(filepath)
      .then(function(collections){
        return import_collections(url(connection), connection.database, filepath, collections);
      })
      .then(function(){
        if(!quiet){
...
function import_collections(host, database, filepath, collections){
  return new Promise(function(resolve, reject){
    var collection = collections.shift();
    if(collections.length === 0) resolve(true);
    ...
    if(collection_info){
    console.log("mongoimport --db="+database+" --host="+host+" --collection="+collection_info[1]+" --file="+collection);
    exec("mongoimport --db="+database+" --host="+host+" --collection="+collection_info[1]+" --file="+collection, function(err, stdout, stderr){
      if(err) reject(stderr)
      resolve(import_collections(host, database, filepath, collections));
    });
    }
    resolve(import_collections(host, database, filepath, collections));
  })
